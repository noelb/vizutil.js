        ...
        value: function getCurve(document, path) {
            ...
            if (isXY) {
                ...
                for (var i = 1; i < path.length; i += 3) {
                    ...
                    if (debug) {
                        group.appendChild(SvgUtil.getControlBar(path[i - 1].x, path[i - 1].y, path[i].x, path[i].y));
                        group.appendChild(SvgUtil.getControlBar(path[i + 2].x, path[i + 2].y, path[i + 1].x, path[i + 1].y));
                    }
                }
            } else {
